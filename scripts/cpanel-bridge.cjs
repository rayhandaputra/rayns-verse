...
// =========================================================================
const BRIDGE_URL = 'https://data.kinau.id/agent-bridge.php';
const AUTH_TOKEN = 'GANTI_DENGAN_TOKEN_RAHASIA_ANDA_YANG_SANGAT_PANJANG_DAN_ACAK';
      ...
      tools: [
        {
          ...
          description: 'Membuat file baru atau menimpa file lama di dalam project workspace cPanel.',
          inputSchema: {
            type: 'object',
            properties: {
              path: { type: 'string', description: 'Relative path file, misal: v1/auth.php' },
              content: { type: 'string', description: 'Isi source code program lengkap' }
            },
        ...
        },
        {
          ...
          inputSchema: {
            type: 'object',
            properties: { path: { type: 'string', description: 'Relative path file, misal: config/db.php' } },
            required: ['path']
          }
        },
        {
          name: 'cpanel_list_directory',
          description: 'Melihat daftar file dan struktur folder di dalam workspace project cPanel.',
          inputSchema: {
            type: 'object',
            properties: { path: { type: 'string', description: 'Relative path folder, kosongkan untuk root workspace' } }
          }
        }
      ]
    });
      ...
      try {
        ...
        if (response.status === 'success') {
          ...
          if (response.content !== undefined) {
            textResult = typeof response.content === 'string' ? response.content : JSON.stringify(response.content, null, 2);
          } else if (response.message !== undefined) {
            textResult = response.message;
          } else if (response.data !== undefined) {
            textResult = typeof response.data === 'string' ? response.data : JSON.stringify(response.data, null, 2);
          }
          sendResponse(id, {
          ...
          sendResponse(id, {
            content: [{ type: 'text', text: `cPanel Bridge Error: ${response.message}` }],
            isError: true
          });
        }
      } catch (err) {
