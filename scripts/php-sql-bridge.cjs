...
// --- Configuration ---
const ENDPOINT = 'https://data.kinau.id/apicore-latest/agent-query';
      ...
      tools: [
        {
          ...
          description: 'Gunakan tool ini untuk mengeksekusi query SQL (SELECT, INSERT, UPDATE, dll.) langsung ke database shared hosting.',
          inputSchema: {
            type: 'object',
            properties: {
              sql: {
                type: 'string',
                description: 'Kueri SQL murni yang ingin dieksekusi (contoh: SELECT * FROM users LIMIT 5;)'
              }
            },
            required: ['sql']
          }
