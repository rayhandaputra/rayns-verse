...
import { NotaPdfTemplate } from "~/components/shared/print/order/NotaTemplateRenderer";
...
export async function generateOrderPdfBuffer(order: any, items: any[]) {
  ...
  const capUrl = path.join(process.cwd(), "public", "capkinau.png");
  // Kirim logoUrl ke template
  // renderToBuffer mengonversi komponen React-PDF menjadi Buffer di server
  const buffer = await renderToBuffer(
    <NotaPdfTemplate order={order} items={items} logoPath={logoUrl} capPath={capUrl} />
  );
  return buffer;
}
