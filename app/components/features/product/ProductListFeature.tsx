// app/components/features/product/ProductListFeature.tsx
...
export const ProductListFeature: React.FC = () => {
  ...
  // Helper to sync drive folders
  const syncDriveFolders = (data: any) => {
    const raw = data?.default_drive_folders;
    let parsed: any[] = [];
  ...
  const handleOpenCatModal = (key: "create" | "update", data: any) => {
    setCatModal({ open: true, key, data });
    syncDriveFolders(data);
  };
  ...
  // --- Tier Logic ---
  const addTier = () => {
    const currentTiers = modal.data?.product_price_rules || [];
    setModal({
      ...modal,
      data: {
        ...modal.data,
  ...
  const updateTier = (idx: number, field: "min_qty" | "price", val: number) => {
    if (!modal.data?.product_price_rules) return;
  ...
  // --- Variation Logic ---
  const addVariation = () => {
    const currentVariants = modal.data?.product_variants || [];
    setModal({
      ...modal,
      data: {
        ...modal.data,
  ...
  const updateVariation = (
    ...
    if (!modal.data?.product_variants) return;
    const copy = [...modal.data.product_variants];

    if (field === "is_default") {
      // If setting as default, uncheck others
      copy.forEach((v, i) => {
        // @ts-expect-error variant may not have is_default in typing but used in JS payload
        ...
        <>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            ...
            <Button
              ...
              onClick={() => handleOpenCatModal("create", null)}
            >
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex justify-end gap-2">
                          <Button
                            ...
                            onClick={() => handleOpenCatModal("update", cat)}
                          >
                            <PencilLine size={14} />
                          </Button>
                          <Button
