...
import type { SubComponent, Shop } from "./types";
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {subComponents.map((sub) => (
                            <div key={sub.id} className={`flex items-center justify-between p-2 pr-3 rounded-xl border transition-all ${cart[`sub:${sub.id}`] ? "bg-blue-50 border-blue-200" : "bg-white border-gray-100"}`}>
                                <span className="text-xs font-bold text-gray-600 pl-2 truncate flex-1">{sub.commodity_name || sub.name}</span>
...
export default function ProcurementShoppingFeature() {
    ...
    const shops: Shop[] = useMemo(() => (shopData?.data?.items || []).filter((s: any) => s.category === category), [shopData, category]);
    const allMaterials: RawMaterial[] = useMemo(() => (matData?.data?.items || []).filter((m: any) => m.category === category), [matData, category]);
    const transactions = useMemo(() => stockLogData?.data?.items || [], [stockLogData]);

    const selectedShopData = useMemo(() => shops.find(s => s.id === selectedShopId), [selectedShopId, shops]);

    const selectedShopMaterials = useMemo(() => {
        if (!selectedShopId) return [];
        return allMaterials.filter(m => m.supplier_id === selectedShopId);
    }, [selectedShopId, allMaterials]);

    const selectedManualMat = useMemo(() => allMaterials.find(m => m.id === manualMatId), [manualMatId, allMaterials]);
    ...
    const calculation = useMemo(() => {
        ...
        Object.entries(cart).forEach(([key, qty]) => {
            ...
            const material = selectedShopMaterials.find(m => m.id === cleanId) ||
    ...
    useEffect(() => {
        if (fetcher.data && (fetcher.data as any).success) {
            ...
            toast.error((fetcher.data as any).message || "Gagal");
        }
    }, [fetcher.data]);
    ...
    const handleCheckout = () => {
        ...
        const payload: any = {
            ...
            is_auto: "true",
            proof: proofImage,
            items: JSON.stringify(calculation.cartItems)
        };
    ...
    const handleManualEntry = () => {
        ...
        if (manualSubId) {
            ...
            if (sub) {
                name += ` [${sub.commodity_name || sub.name}]`;
                targetId = manualSubId;
                isParent = 0;
            }
        }
                    ...
                    <div className="p-8 space-y-8">
                        <div>
                            ...
                            <ReactSelect
                                ...
                                value={shops.map(s => ({ value: s.id, label: `${s.name} (${s.type})` })).find(o => o.value === selectedShopId)}
                                onChange={(val: any) => {
                            ...
                            <div className="space-y-8 animate-fade-in">
                                ...
                                <div className="bg-gray-900 rounded-[32px] p-8 text-white flex flex-col md:flex-row justify-between items-center gap-6 shadow-2xl">
                                    ...
                                    <button
                                        ...
                                        disabled={fetcher.state !== "idle" || calculation.cartItems.length === 0}
                                    ...
                                    >
                                        {fetcher.state !== "idle" ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Check size={20} />}
                                        {fetcher.state !== "idle" ? "Menyimpan..." : "Simpan Transaksi"}
                                    </button>
                                </div>
                            </div>
                        )}
                ...
                <div className="bg-white rounded-[32px] border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <div className="p-8 space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {selectedManualMat && +selectedManualMat.is_package === 1 && (
                                <select
                                    className="w-full border-2 border-gray-100 p-4 rounded-2xl text-sm font-bold bg-white focus:border-orange-300 outline-none"
                                    value={manualSubId}
                                ...
                                >
                                    ...
                                    {(safeParseArray(selectedManualMat?.sub_components) as SubComponent[]).map(s => (
                                        <option key={s.id} value={s.id}>{s.commodity_name || s.name}</option>
                                    ))}
                                </select>
                            )}
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-4 items-end">
                            ...
                            <button
                                ...
                                disabled={!manualMatId || !manualQty || fetcher.state !== "idle"}
                            ...
                            >
                                {fetcher.state !== "idle" ? "..." : <><PlusCircle size={20} /> TAMBAH</>}
                            </button>
                        </div>
                    </div>
                </div>
