...
import { Wallet, TrendingUp, TrendingDown, Plus, Monitor, Tag, CreditCard, Trash2, Edit2, Image, X, Download, Filter, PieChart as PieIcon, BarChart3 } from 'lucide-react';
...
const FinancePage: React.FC<FinancePageProps> = ({
  ...
  const { data: transactionsData, loading: transactionsLoading, reload: reloadTransactions } = useFetcherData<any>({
  ...
  React.useEffect(() => {
    if (actionFetcher.data) {
      ...
      if (result.success) {
        ...
        setTxForm({ type: 'Expense', date: new Date().toISOString().split('T')[0] });
        setProofImage('');
        setBankForm({});
                        ...
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {transactionsLoading ? (
                                    ...
                                    <tr><td colSpan={5} className="text-center py-6 text-gray-400">Belum ada transaksi untuk tahun {filterYear}.</td></tr>
                                ) : (
                                    transactions.map(t => (
                                        <tr key={t.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-3 text-gray-500 text-xs">{t.bank_id || '-'}</td>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {banks.map(b => {
                                ...
                                <tr key={b.id} className="hover:bg-gray-50">
                                    <td className="px-6 py-3 font-bold text-gray-800">{b.bank_name}</td>
                                    <td className="px-6 py-3 font-mono text-gray-600">{b.account_number}</td>
                                    <td className="px-6 py-3 text-gray-600">{b.holder_name}</td>
                    ...
                    <form onSubmit={handleAddTx} className="space-y-4">
                        ...
                        <div>
                            ...
                            >
                                ...
                                {banks.map(b => (
                                    <option key={b.id} value={`${b.bank_name} - ${b.holder_name}`}>
