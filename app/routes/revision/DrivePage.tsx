...
import { DriveItem, Order } from '../types';
import { 
  Folder, File, MoreVertical, Trash2, Edit2, FolderPlus, 
  Upload, Scissors, Clipboard, Share2, CornerUpLeft, HardDrive,
  FileImage, FileText, Receipt, Layout, ArrowLeft, Check, X, Shield, Users
} from 'lucide-react';
...
const DrivePage: React.FC<DrivePageProps> = ({ items, orders = [], onUpdateItems, initialFolderId, rootFolderId, isGuest = false, onSaveReview }) => {
  ...
  // New Tab for Drive Sections (Only applicable if rootFolderId is NOT set)
  const [driveSection, setDriveSection] = useState<'customer' | 'internal'>('customer');

  const [selectedItem, setSelectedItem] = useState<string | null>(null);
  const [clipboard, setClipboard] = useState<{ id: string, op: 'cut' | 'copy' } | null>(null);
  ...
  const currentOrder = effectiveOrderId ? orders.find(o => o.driveFolderId === effectiveOrderId) : null;

  // --- FILTER ITEMS BASED ON TAB & FOLDER ---
  let currentItems = items.filter(item => item.parentId === currentFolderId);

  // Special Logic for Root Level (No Folder ID and No Locked Root)
  if (currentFolderId === null && !rootFolderId) {
      if (driveSection === 'customer') {
          // Show only folders that match an Order's driveFolderId
          const orderFolderIds = new Set(orders.map(o => o.driveFolderId));
          currentItems = items.filter(item => 
              item.parentId === null && orderFolderIds.has(item.id)
          );
      } else {
          // Show items that are NOT linked to orders (Internal)
          const orderFolderIds = new Set(orders.map(o => o.driveFolderId));
          currentItems = items.filter(item => 
              item.parentId === null && !orderFolderIds.has(item.id)
          );
      ...
      // Inside a folder, just show children
      currentItems = items.filter(item => item.parentId === currentFolderId);
  }
  
  // Sort: Folders first, then new to old
  currentItems.sort((a, b) => {
    if (a.type === b.type) return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
  ...
  const confirmCreateFolder = (e?: React.FormEvent) => {
    ...
    const newFolder: DriveItem = {
      id: String(Date.now()),
      parentId: currentFolderId, // If null, creates in root based on current logic
  ...
  const handleRenameSave = () => {
    if (isRenaming && renameValue.trim()) {
      onUpdateItems(items.map(i => i.id === isRenaming ? { ...i, name: renameValue } : i));
    }
    setIsRenaming(null);
  };
  ...
  const handlePaste = () => {
    ...
    if (!item) { setClipboard(null); return; }
    ...
    if (clipboard.op === 'cut') {
      onUpdateItems(items.map(i => i.id === clipboard.id ? { ...i, parentId: currentFolderId } : i));
    }
    setClipboard(null);
  };
  ...
  const handleShare = async (item?: DriveItem) => {
    ...
    if (!targetId) {
        alert("Tidak dapat membagikan root drive utama. Masuk ke dalam folder pesanan terlebih dahulu.");
        return;
    }
  ...
  const renderIcon = (item: DriveItem) => {
      if (item.type === 'folder') return <Folder size={48} className="text-yellow-400 fill-yellow-400" />;
      if (item.mimeType === 'image') return <FileImage size={40} className="text-purple-500" />;
  ...
  return (
    <div className={`bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col ${isGuest ? 'h-screen rounded-none border-0' : 'h-[calc(100vh-140px)]'}`}>
          ...
          <div className="p-4 bg-gray-900 text-white flex justify-between items-center shadow-md">
             <div className="flex items-center gap-3">
                 ...
                 <div>
                     ...
                     <p className="text-xs text-gray-400">File Storage & Order Details</p>
                 </div>
             </div>
             {currentOrder && (
      ...
      {/* MAIN DRIVE TABS (Only if NOT locked to order root) */}
      {!currentOrder && !isGuest && (
          <div className="flex border-b border-gray-200">
              <button
                onClick={() => { setDriveSection('customer'); setCurrentFolderId(null); }}
                className={`flex-1 py-3 text-sm font-medium flex justify-center items-center gap-2 border-b-2 transition ${driveSection === 'customer' ? 'border-blue-600 text-blue-700 bg-blue-50/50' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}
              >
                  <Users size={16}/> Data Customer
              </button>
              <button
                onClick={() => { setDriveSection('internal'); setCurrentFolderId(null); }}
                className={`flex-1 py-3 text-sm font-medium flex justify-center items-center gap-2 border-b-2 transition ${driveSection === 'internal' ? 'border-purple-600 text-purple-700 bg-purple-50/50' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}
              >
      ...
      {activeTab === 'nota' && currentOrder ? (
          <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-gray-50">
              <NotaView 
                order={currentOrder} 
                onSaveReview={onSaveReview ? (r, t) => onSaveReview(currentOrder.id, r, t) : undefined}
              />
          </div>
      ) : (
        <>
            ...
            <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-2 text-sm text-gray-600 overflow-x-auto">
                {!rootFolderId && (
                     ...
                     >
                        <HardDrive size={14} className="mr-1" /> {driveSection === 'customer' ? 'Customer Root' : 'Internal Root'}
                     </button>
                )}
                {getBreadcrumbs().map((crumb, idx) => (
                    <React.Fragment key={crumb.id}>
                        {(!rootFolderId || idx > 0) && <span className="text-gray-300">/</span>}
                        <button 
                        ...
                        {currentItems.map(item => (
                            <div 
                                ...
                                className={`group relative p-4 rounded-xl border flex flex-col items-center gap-3 cursor-pointer transition-all ${
                                    selectedItem === item.id ? 'bg-blue-50 border-blue-400 ring-1 ring-blue-400' : 'bg-white border-gray-100 hover:border-gray-300 hover:shadow-sm'
                            ...
                            >
                                ...
                                {isRenaming === item.id ? (
                                    ...
                                    <div className="text-center w-full">
                                        ...
                                        {item.type === 'file' && <div className="text-[10px] text-gray-400 mt-1">{item.size}</div>}
                                    </div>
                                )}

                                {/* Context Menu */}
                                <div className={`absolute top-2 right-2 flex flex-col bg-white border border-gray-200 rounded-lg shadow-lg z-10 overflow-hidden transition-all ${selectedItem === item.id ? 'opacity-100 visible' : 'opacity-0 invisible group-hover:visible group-hover:opacity-100'}`}>
                                    {item.type === 'folder' && (
                                        <button onClick={(e) => {e.stopPropagation(); handleShare(item)}} title="Bagikan Link" className="p-1.5 hover:bg-gray-100 text-blue-600 border-b border-gray-100">
                                            <Share2 size={12}/>
                                        </button>
                                    )}
                                    <button onClick={(e) => {e.stopPropagation(); handleRenameStart(item)}} title="Ganti Nama" className="p-1.5 hover:bg-gray-100 text-gray-600"><Edit2 size={12}/></button>
                                    <button onClick={(e) => {e.stopPropagation(); handleCut(item.id)}} title="Pindahkan" className="p-1.5 hover:bg-gray-100 text-orange-600"><Scissors size={12}/></button>
                                    <button onClick={(e) => {e.stopPropagation(); handleDelete(item.id)}} title="Hapus" className="p-1.5 hover:bg-gray-100 text-red-600"><Trash2 size={12}/></button>
                                </div>
                            </div>
                        ))}
                    </div>
      ...
      {showNewFolderModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
              <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6">
                  <h3 className="text-lg font-bold text-gray-800 mb-4">Buat Folder Baru</h3>
