...
import { formatCurrency, formatFullDate, getGoogleMapsLink, getWhatsAppLink, ADMIN_WA } from '../constants';
import { MapPin, Phone, Printer, CheckCircle, Star, Handshake, Image, X, MessageSquare } from 'lucide-react';
...
const NotaView: React.FC<NotaViewProps> = ({ order, onSaveReview }) => {
  ...
  const PaymentProofModal = () => {
      ...
      return (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4 animate-fade-in no-print" onClick={() => setShowProofModal(false)}>
              <div className="bg-white rounded-xl p-4 max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                  <div className="flex justify-between items-center mb-4 border-b border-gray-100 pb-2">
                      ...
                      <button onClick={() => setShowProofModal(false)} className="p-1 hover:bg-gray-100 rounded-full"><X size={20} /></button>
                  </div>
  ...
  return (
    ...
    <div className="bg-white p-6 max-w-lg mx-auto font-sans text-gray-800 print-container relative">
      ...
      <div className="flex justify-between items-start border-b-2 border-gray-800 pb-4 mb-4">
        <div>
           <div className="flex items-center gap-2 mb-1">
               <div className="w-8 h-8 bg-gray-900 text-white flex items-center justify-center font-bold rounded">K</div>
               <h1 className="text-xl font-bold tracking-tight">Kinau.id</h1>
           </div>
           <p className="text-xs text-gray-500">Your Custom Specialist</p>
        </div>
        <div className="text-right">
           <h2 className="text-lg font-bold text-gray-400 uppercase tracking-widest">NOTA</h2>
           <p className="text-xs font-mono text-gray-500">#{order.id.slice(-6)}</p>
           <p className="text-xs text-gray-500">{formatFullDate(order.createdAt)}</p>
        </div>
      </div>
      ...
      <div 
        ...
        style={{
            backgroundColor: settings.headerBackground ? 'transparent' : '#f9fafb',
        }}
      >
        {settings.headerBackground && (
            <div className="absolute inset-0 z-0">
                <img src={settings.headerBackground} className="w-full h-full object-cover opacity-15 print:opacity-10" />
                <div className="absolute inset-0 bg-white/50"></div>
            </div>
        )}

        <div className="relative z-10">
            <h3 className="text-xs font-bold text-gray-400 uppercase mb-2">{headerLabel}</h3>
            ...
            {order.isKKN && order.kknDetails && (
                <p className="text-xs text-blue-600 mt-1 font-medium bg-blue-50 w-fit px-2 py-0.5 rounded print:text-gray-600 print:bg-transparent print:p-0">
                    KKN ITERA {order.kknDetails.tahun} - Periode {order.kknDetails.periode}
                </p>
            )}
            ...
            <div className="mt-2 text-sm text-gray-600">
                {isPerorangan ? (
                    ...
                    <>
                        <div className="font-medium text-gray-500 text-xs uppercase mb-0.5">{subLabel}</div>
                        <div>{order.pemesanName} <span className="text-gray-400">({order.pemesanPhone})</span></div>
                    </>
                )}
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200/50 grid grid-cols-2 gap-4">
                <div>
                    <span className="block text-gray-400 text-xs mb-1">Status Produksi</span>
                ...
                <div>
                    <span className="block text-gray-400 text-xs mb-1">Status Pembayaran</span>
                    <div className="flex flex-col items-start">
                        <span className={`font-bold text-sm px-2 py-1 rounded print:bg-transparent print:p-0 print:text-black print:border print:border-gray-300 ${
                            order.statusPembayaran === 'Lunas' ? 'bg-green-100 text-green-700' :
                            order.statusPembayaran === 'DP' ? 'bg-yellow-100 text-yellow-700' :
                            'bg-red-100 text-red-600'
                        }`}>
                            {order.statusPembayaran === 'Tidak Ada' ? 'BELUM BAYAR' : 
                            order.statusPembayaran === 'Lunas' ? 'LUNAS' : 'DP (BELUM LUNAS)'}
                        </span>
                        {hasProof && (
                            <span className="text-[10px] text-gray-500 mt-0.5 flex items-center gap-1">
                                <CheckCircle size={10} className="text-green-500"/> Bukti Uploaded
                            </span>
                        )}
                    </div>
                </div>
            ...
            <div className="mt-3 text-xs pt-2 border-t border-gray-200/50">
                ...
                <span className="font-semibold text-red-600 print:text-black">{formatFullDate(order.deadline)}</span>
            </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        <table className="w-full text-sm">
           <thead className="border-b border-gray-200">
              <tr>
                 <th className="text-left py-2 font-semibold text-gray-600">Produk</th>
                 <th className="text-right py-2 font-semibold text-gray-600">Qty</th>
                 <th className="text-right py-2 font-semibold text-gray-600">Harga</th>
                  ...
                  order.items.map((item, idx) => (
                    <tr key={idx}>
                        <td className="py-3 font-medium">
                            {item.productName}
                            {item.variationName && <span className="font-normal text-gray-600"> ({item.variationName})</span>}
                        </td>
                        ...
                        {order.customItems && order.customItems.length > 0 && (
                            <div className="text-xs text-gray-500 mt-1">
                                {order.customItems.map((item, i) => (
                                    <div key={i}>+ {item.name} ({item.quantity} pcs)</div>
                                ))}
                            </div>
                        )}
                    </td>
                    <td className="py-3 text-right">{order.jumlah}</td>
                    <td className="py-3 text-right">{formatCurrency(order.unitPrice)}</td>
          ...
          <div className="w-3/5 space-y-2">
                 ...
                 <div className="flex justify-between text-sm text-green-600 print:text-black">
                    ...
                    <span>- {formatCurrency(
                        order.discount.type === 'nominal' 
                            ...
                            : (order.totalAmount / (1 - order.discount.value/100)) * (order.discount.value/100) // approx reverse calc
                    )}</span>
                 </div>
             )}
             ...
             {order.isSponsor && (
                 <div className="flex justify-between text-sm text-purple-600 font-bold bg-purple-50 p-2 rounded print:bg-transparent print:text-black print:p-0">
                    <span className="flex items-center gap-1"><Handshake size={14}/> SPONSORSHIP / PARTNER</span>
                 </div>
             )}
             ...
             <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-green-600 print:text-black">{formatCurrency(paid)}</span>
             </div>
             <div className="flex justify-between text-base border-t border-gray-200 pt-2">
                <span className="font-bold">Sisa Pembayaran</span>
                <span className={`font-bold ${isPaidOff ? 'text-green-600' : 'text-red-600'} print:text-black`}>
      ...
      {order.review ? (
          <div className="mb-6 p-4 bg-yellow-50 border border-yellow-100 rounded-lg print:bg-white print:border-gray-200">
              <h3 className="text-xs font-bold text-gray-500 uppercase mb-2">Ulasan Pelanggan</h3>
              <div className="flex text-yellow-400 mb-1 print:text-black">
                  {Array.from({length: order.rating || 5}).map((_, i) => <Star key={i} size={14} fill="currentColor" />)}
              </div>
              ...
              <div className="mb-6 p-6 bg-white border border-gray-200 rounded-xl shadow-sm no-print">
                  ...
                  <form onSubmit={handleReviewSubmit}>
                      ...
                      <textarea 
                          ...
                          required
                      />
                      <button type="submit" className="w-full bg-gray-900 text-white py-2 rounded-lg text-sm font-medium hover:bg-gray-800 transition">
                          Kirim Ulasan
                      </button>
                  </form>
              </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4 no-print">
          ...
          <a 
            href={getWhatsAppLink(ADMIN_WA, `Halo Admin, saya ingin konfirmasi pesanan #${order.id.slice(-4)} (${order.instansi})`)}
            target="_blank" 
            rel="noreferrer"
            className="flex items-center justify-center gap-2 py-2 px-3 bg-white border border-gray-300 rounded-lg text-xs font-semibold text-gray-700 hover:bg-gray-50"
          >
