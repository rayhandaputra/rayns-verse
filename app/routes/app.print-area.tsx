...
// Diperbarui sesuai kebutuhan UI Kode 2
interface PrintSlot {
  id: string;
  fileId: string;
  fileName: string;
  ...
  hookColor: string;
  isMasterColor: boolean;
  side?: 1 | 2; // New from Kode 2
  isBack?: boolean; // New from Kode 2
}
...
// --- Helper Components (Modularized & UI Enhanced) ---
...
// 1. A4 Sheet Renderer (ID Cards) - UI Updated from Kode 2
const A4Sheet: React.FC<{
  pageSlots: PrintSlot[];
  pageIndex: number;
  totalSlots: PrintSlot[]; // Passed for counting
  onRemove: (globalIndex: number) => void;
  onClone: (globalIndex: number) => void;
}> = ({ pageSlots, pageIndex, totalSlots, onRemove, onClone }) => {
  const getDisplayNumber = (currIdx: number) => currIdx + 1; // Simplifikasi numbering
            ...
            >
              <div
                ...
                style={{
                  width: "56mm",
                  height: "88mm",
                  boxSizing: "border-box", // From Kode 2
                }}
              >
                {slot?.data && (
                  <img
...
// 2. Lanyard Sheet Renderer - Full UI Upgrade from Kode 2
const LanyardSheet: React.FC<{
  pageSlots: PrintSlot[];
  pageIndex: number;
  totalSlots: PrintSlot[];
  onRemove: (index: number) => void;
  onClone: (index: number) => void;
  onCopyByCard: (index: number) => void;
  onColorChange: (id: string, color: string) => void;
  ...
  onColorChange,
    ...
    return (
      <div className="lanyard-sheet bg-white shadow-2xl mx-auto border border-gray-200 relative mb-20 overflow-hidden print:shadow-none print:border-0 print:m-0">
        {/* Penomoran Lembar (Hanya Muncul di Layar) */}
        ...
        {/* Garis Bantu CMYK (Guides) - Dibuat sangat tipis agar tidak merusak presisi */}
        <div className="absolute inset-0 pointer-events-none">
          {[
            ...
            <div key={i} className="absolute left-0 right-0" style={{ top: line.top, height: "0.2mm", backgroundColor: line.color, opacity: 0.5 }} />
          ))}
          {/* Garis Tengah Gradient */}
          <div className="absolute left-0 right-0" style={{ top: "calc(15mm + 500mm)", height: "0.5mm", background: "linear-gradient(to right, #000, #ff0, #f0f, #0ff)" }} />
        </div>

        {/* Flex Container Utama Slot */}
        <div
              ...
              <div key={idx} className="flex flex-col items-center">
                <div
                  ...
                  style={{
                    ...
                    outline: "0.1mm solid #e5e7eb", // Menggunakan outline agar tidak menambah lebar 22mm
                    boxSizing: "content-box",
                  }}
                >
                  {slot ? (
                    <>
                      {/* Area Desain Lanyard (90cm) */}
                      <div className="relative overflow-hidden" style={{ width: "22mm", height: "900mm" }}>
                        {slot.data && (
                          <img
                            src={slot.data}
                            alt="Lanyard"
                      ...
                      {/* Garis Potong / Lipat Putih Tipis */}
                      <div style={{ width: "100%", height: "0.2mm", backgroundColor: "#fff", zIndex: 5 }} />
                      ...
                      {/* Action Overlay (Hanya Layar) */}
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition no-print flex flex-col items-center justify-start pt-10 gap-4 z-30">
                        <button onClick={() => onCopyByCard(globalIndex)} className="bg-blue-600 text-white p-4 rounded-full shadow-xl hover:scale-110"><Copy size={20} /></button>
                        <button onClick={() => onClone(globalIndex)} className="bg-emerald-600 text-white p-4 rounded-full shadow-xl hover:scale-110"><Plus size={20} /></button>
                        {slot.isMasterColor && (
                          <div className="relative">
                            <button className="bg-violet-600 text-white p-4 rounded-full shadow-xl"><Palette size={20} /></button>
                            <input type="color" className="absolute inset-0 opacity-0 cursor-pointer" value={slot.hookColor} onChange={(e) => onColorChange(slot.id, e.target.value)} />
                          </div>
                        )}
                        <button onClick={() => onRemove(globalIndex)} className="bg-red-600 text-white p-4 rounded-full shadow-xl mt-6"><Trash2 size={20} /></button>
                      </div>
                    </>
                  ) : (
                    <div className="text-[10px] text-gray-300 font-bold rotate-90 h-full flex items-center justify-center no-print uppercase tracking-widest">KOSONG</div>
                  )}
                </div>
                {/* Penomoran Slot di Bawah (Hanya Layar) */}
...
export default function PrintPage() {
  ...
  // CSS Updated from Kode 2 (Cleaned and merged)
  const printStyles = `
    @media print {
      @page { 
        size: 210mm 1032mm; 
      ...
      .lanyard-sheet { 
        ...
        overflow: hidden !important; /* Mencegah elemen guide meluber */
      }

      /* Khusus ID Card tetap A4 */
      .category-idcard .lanyard-sheet { display: none; } 
    }
  `;

  // 2. Data Fetching (Keep Kode 1 Logic)
  const { data: getOrders, reload: reloadOrders } = useFetcherData({
    endpoint: nexus()
      .module("ORDERS")
      .action("get")
  ...
  // --- Logic Improved based on Kode 2 ---
  const copyByCardFileCount = async (index: number) => {
    const source = state.lanyardSlots[index];
    ...
    // 2. Cari folder ID Card Depan (Improved logic from Kode 2)
    const cardFolder: any = allFolders.find(
      (f: any) =>
        f.folder_name.toLowerCase().includes("id card") &&
        (f.folder_name.toLowerCase().includes("depan") ||
    ...
    if (!cardFolder) {
      toast.error(
        "Folder ID Card (Depan) tidak ditemukan untuk referensi jumlah."
      );
      return;
    }
    ...
    // 3. Ambil file count
    const cardFiles = await getFiles(cardFolder?.id);
    const count = cardFiles.length; // Use exact count, not length - 1, logic from Kode 2

    if (count === 0) return;

    // 4. Cek varian 2 sisi (Logic from Kode 2)
    const isTwoSided = safeParseArray(order.order_items)?.some((it: any) =>
      it?.variant_name?.toLowerCase().includes("2 sisi")
    );
    ...
    // Replace current single slot with batch, or append?
    // Kode 2 replaces the clicked slot with the batch.
    setState((prev) => {
      const next = [...prev.lanyardSlots];
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] overflow-hidden gap-6">
      {/* Sidebar - UI Refined from Kode 2 */}
      <aside className="w-80 bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col flex-shrink-0 no-print">
        <div className="p-4 border-b border-gray-100 flex items-center justify-between">
          <h3 className="font-bold text-gray-800 flex items-center gap-2">
            <Layers size={18} className="text-blue-600" /> Sumber Cetak
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50 rounded-b-xl">
          ...
          {PrintButton ? (
            <PrintButton
              externalRef={printAreaRef}
              label={`Cetak ${state.category.toUpperCase()} - ${moment().format("MM-DD-YYYY (HH:mm)")}`}
              pageStyle={printStyles} // Use the consolidated style
            >
              {({ handlePrint }: any) => (
                <button
                  onClick={handlePrint}
      ...
      {/* Main Canvas - Custom Scrollbar logic from Kode 2 */}
      <main className="flex-1 bg-gray-100 rounded-xl border border-gray-200 shadow-inner overflow-y-auto p-10 flex flex-col items-center custom-print-scroll">
        <div ref={printAreaRef} className="flex flex-col items-center">
          {state.category === "lanyard"
            ? chunkedLanyardPages.map((page, idx) => (
        ...
        <div className="mt-4 text-center text-gray-400 text-[10px] font-medium max-w-sm pb-10 no-print">
          ...
          permanen di tiap slot lanyard (area desain murni tetap 100x2.2cm).
        </div>
      </main>

      <style>{`
        .custom-print-scroll::-webkit-scrollbar { width: 6px; }
        .custom-print-scroll::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 10px; }
      `}</style>
    </div>
  );
}
