...
- **Pattern saat ini:**
  ```ts
  const url = "https://data.kinau.id/mailbox.php";
...
- **Pattern saat ini:** `fetch(mailbox_url)`, `fetch("https://data.kinau.id/send_email.php", { method: "POST", body })`
...
```tsx
// ❌ Pola lama
const { data } = useFetcherData({ endpoint: nexus().module("ORDERS").action("get").params({ page: 0, size: 10 }).build() });

// ✅ Pola baru
const { data } = useFetcherData({ endpoint: "/api/nexus", params: { module: "ORDERS", action: "get", page: 0, size: 10 } });
```
