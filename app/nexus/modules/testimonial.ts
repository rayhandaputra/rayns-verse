...
export const TestimonialAPI = {
  ...
  get: async ({ session, req }: any) => {
    ...
    try {
      const result = await APIProvider(session)
        .Endpoint("POST", "select", "testimonials")
        .Data({
          columns: [
            "id",
            "order_number",
  ...
  create: async ({ session, req }: any) => {
    ...
    try {
      const result = await APIProvider(session)
        .Endpoint("POST", "insert", "testimonials")
        .Data({ data: newTestimonial })
        .Result();
  ...
  update: async ({ session, req }: any) => {
    ...
    if (!id) {
      return {
        success: false,
        message: "ID testimonial wajib diisi for update",
      };
    }
    ...
    try {
      const result = await APIProvider(session)
        .Endpoint("POST", "update", "testimonials")
        .Data({
