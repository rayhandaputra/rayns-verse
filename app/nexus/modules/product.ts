...
export const ProductAPI = {
    ...
    try {
      ...
      if (Array.isArray(items) && items.length > 0) {
        await APIProvider(session)
          .Endpoint("POST", "bulk-insert", "product_components")
          .Data({
      ...
      if (Array.isArray(variants) && variants.length > 0) {
        ...
        await APIProvider(session)
          .Endpoint("POST", "bulk-insert", "product_variants")
          .Data({
            updateOnDuplicate: true,
            rows: variantRows,
          })
  ...
  update: async ({ session, req }: any) => {
    ...
    try {
      ...
      if (Array.isArray(price_rules)) {
        ...
        const delPrice = await APIProvider(session)
        ...
        if (price_rules.length > 0) {
          ...
          const resPriceRule = await APIProvider(session)
            .Endpoint("POST", "bulk-insert", "product_price_rules")
            .Data({
              updateOnDuplicate: true,
              rows: priceRuleRows,
      ...
      if (Array.isArray(variants)) {
        // First, soft delete existing price rules for this product
        const resUt = await APIProvider(session)
          .Endpoint("POST", "update", "product_variants")
          .Data({
            data: { deleted_on: new Date().toISOString() },
            where: { product_id: id },
          })
        ...
        if (variants.length > 0) {
          ...
          const resVariant = await APIProvider(session)
            .Endpoint("POST", "bulk-insert", "product_variants")
            .Data({
              updateOnDuplicate: true,
              rows: variantRows,
  ...
  // ============================================================
  bulkInsert: async ({ session, req }: any) => {
    ...
    try {
      ...
      for (const productData of products) {
        ...
        // Insert product
        const productResult = await APIProvider(session)
          .Endpoint("POST", "insert", "products")
          .Data({ data: newProduct })
          .Result();
