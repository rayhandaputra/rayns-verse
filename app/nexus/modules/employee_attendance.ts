...
export const EmployeeAttendanceAPI = {
  get: async ({ session, req }: any) => {
    ...
    return APIProvider(session)
      .Endpoint("POST", "select", "employee_attendances")
      .Data({
        columns: [
          "id",
          "employee_id",
  ...
  create: async ({ session, req }: any) => {
    ...
    const result = await APIProvider(session)
      .Endpoint("POST", "insert", "employee_attendances")
      .Data({
        data: {
          employee_id,
          employee_name,
  ...
  clockOut: async ({ session, req }: any) => {
    ...
    const attendanceCheck = await APIProvider(session)
      .Endpoint("POST", "select", "employee_attendances")
      .Data({
        columns: ["id", "time_out"],
        where: {
    ...
    const result = await APIProvider(session)
      .Endpoint("POST", "update", "employee_attendances")
      .Data({
        data: {
