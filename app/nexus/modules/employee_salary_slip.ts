...
export const EmployeeSalarySlipAPI = {
  get: async ({ session, req }: any) => {
    ...
    return await APIProvider(session)
      .Endpoint("POST", "select", "employee_salary_slips")
      .Data({
        columns: [
          "id",
          "employee_id",
  ...
  create: async ({ session, req }: any) => {
    ...
    const result = await APIProvider(session)
      .Endpoint("POST", "insert", "employee_salary_slips")
      .Data({
        data: {
          employee_id,
          employee_name,
  ...
  update: async ({ session, req }: any) => {
    ...
    if (
      ...
      const current = await APIProvider(session)
        .Endpoint("POST", "select", "employee_salary_slips")
        .Data({
    ...
    const result = await APIProvider(session)
      .Endpoint("POST", "update", "employee_salary_slips")
      .Data({
        data: updatedData,
        where: { id },
      })
