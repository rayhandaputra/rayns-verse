...
export const OverviewAPI = {
  ...
  // ============================================================
  get: async ({ session, req }: any): Promise<OverviewResponse> => {
    ...
    try {
      ...
      // 1. MAIN STATS
      // ============================================
      const statsResult = await APIProvider(session)
      ...
      // ============================================
      const monthlyResult = await APIProvider(session)
        .Endpoint("POST", "select", "orders")
        .Data({
          columns: [
      ...
      // 3. STATUS BREAKDOWN
      // ============================================
      const statusResult = await APIProvider(session)
        .Endpoint("POST", "select", "orders")
        .Data({
          columns: [
            "status",
      ...
      // 4. PAYMENT BREAKDOWN
      // ============================================
      const paymentResult = await APIProvider(session)
        .Endpoint("POST", "select", "orders")
        .Data({
          columns: [
            "payment_status",
      ...
      // ============================================
      const topInstResult = await APIProvider(session)
        .Endpoint("POST", "select", "orders")
        .Data({
          columns: [
            "institution_name",
      ...
      // ============================================
      const recentResult = await APIProvider(session)
        .Endpoint("POST", "select", "orders")
        .Data({
          columns: [
            "id",
  ...
  summary: async ({ session, req }: any) => {
    try {
      const order = await APIProvider(session)
        .Endpoint("POST", "select", "orders")
        .Data({
          columns: [
      ...
      const quartarlySixMonths = await APIProvider(session)
        .Endpoint("POST", "select", "orders")
        .Data({
  ...
  getStats: async ({ session, req }: any): Promise<OverviewStats> => {
    try {
