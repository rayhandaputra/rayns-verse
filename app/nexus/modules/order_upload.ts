...
export const OrderUploadAPI = {
  ...
  // ✅ GET FOLDER LIST
  // ============================================================
  get_folder: async ({ session, req }: any) => {
    const {
      pagination = "true",
      page = 0,
      size = 10,
    ...
    try {
      const result = await APIProvider(session)
        .Endpoint("POST", "select", "order_upload_folders")
        .Data({
          columns: [
            "id",
            "uid",
  ...
  // ✅ GET FILE LIST
  // ============================================================
  get_file: async ({ session, req }: any) => {
    const {
      pagination = "true",
      page = 0,
      size = 10,
    ...
    try {
      const result = await APIProvider(session)
        .Endpoint("POST", "select", "order_upload_files")
        .Data({
          columns: [
            "id",
            "code",
  ...
  // ✅ GET TOTAL FILE
  // ============================================================
  get_total_file_twibbon: async ({ session, req }: any) => {
    const {
      order_number,
      folder_id,
    } = req.query || {};
    ...
    try {
      for (const folder of folders) {
        ...
        if (!id || id.toString().includes("KEY")) {
          folderResult = await APIProvider(session)
            .Endpoint("POST", "insert", "order_upload_folders")
            .Data({ data: folderData })
            .Result();
        } else {
          folderResult = await APIProvider(session)
            .Endpoint("POST", "update", "order_upload_folders")
            .Data({
              data: folderData,
              where: { id },
            })
        ...
        if (files && files.length > 0) {
          ...
          await APIProvider(session)
            .Endpoint("POST", "bulk-insert", "order_upload_files")
            .Data({
              rows,
  ...
  // ============================================================
  create_single_folder: async ({ session, req }: any) => {
    const {
      id,
      order_number,
      folder_name,
    ...
    try {
      ...
      if (!id) {
        const insertResult = await APIProvider(session)
          .Endpoint("POST", "insert", "order_upload_folders")
          .Data({ data: folderData })
          .Result();
  ...
  // ✅ CREATE SINGLE FILE
  // ============================================================
  create_single_file: async ({ session, req }: any) => {
    const {
      id,
      folder_id,
      order_number,
    ...
    try {
      ...
      if (!id || id.toString().includes("KEY")) {
        ...
        const insertResult = await APIProvider(session)
          .Endpoint("POST", "insert", "order_upload_files")
          .Data({ data: fileData })
          .Result();
      ...
      // update
      await APIProvider(session)
        .Endpoint("POST", "update", "order_upload_files")
        .Data({
          data: fileData,
          where: { id },
        })
  ...
  // ============================================================
  delete_folder: async ({ session, req }: any) => {
    const { id } = req.body || {};
  ...
  // ============================================================
  delete_file: async ({ session, req }: any) => {
    const { id } = req.body || {};
  ...
  // ============================================================
  bulk_delete_folders: async ({ session, req }: any) => {
    const { ids } = req.body || {};
  ...
  // ============================================================
  bulk_delete_files: async ({ session, req }: any) => {
    const { ids } = req.body || {};
