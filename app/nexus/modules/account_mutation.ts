...
export const AccountMutationAPI = {
  get: async ({ session, req }: any) => {
    ...
    return APIProvider(session)
      .Endpoint("POST", "select", "account_ledger_mutations")
      .Data({
        columns: [
          "id",
          "account_code",
        ...
        where: {
          ...
          // ...(year ? { "year:trx_date": parseInt(year) } : {}),
  ...
  //   });
  // },
  getFinanceReport: async ({ session, req }: any) => {
    ...
    const response = await APIProvider(session)
      .Endpoint("POST", "select", "account_ledger_mutations")
      .Data({
        columns: [
          `MONTH(created_on) as month`,
  ...
  getExpenseComposition: async ({ session, req }: any) => {
    const { year = new Date().getFullYear() } = req.query || {};
